<template>
  <div outlined fluid v-if="data" ref="container" class="chat pa-0">
    ...
    <v-dialog v-model="addUserModal" max-width="600px">
      <v-card class="pa-4">
        <v-autocomplete
          multiple
          chips
        ...
        <div class="d-flex align-center">
          <v-btn
            ...
            >{{ joinLink
            }}<v-icon right ref="copyBtn" v-text="coppiedIcon"></v-icon
          ></v-btn>
        </div>
      </v-card>
    </v-dialog>
...
export default {
  ...
  computed: {
    joinLink() {
      return `http://localhost:8080/join?id=${this.data._id}`;
    },
    cerrado() {
  ...
  methods: {
    ...
    async getAllUsers() {
      ...
      this.allUsers = users.filter(
        (user) => !this.data.user.find((u) => u._id == user._id)
      );
    },
  },
  async mounted() {

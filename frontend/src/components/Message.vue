		...
		>
			<v-avatar class="mb-5">
				<v-img
					...
					:src="`http://localhost:3000/avatars/${img}`"
				></v-img>
			</v-avatar>
		</v-col>
					...
					<div style="display: grid" class="ma-1">
						<template v-if="file && file._id">
							<div v-viewer="{ navbar: false, toolbar: false }">
								<img
									v-if="types[file.ext].includes('image')"
									:src="`http://localhost:3000/documents/_${file._id}.${file.ext}`"
...
export default {
	...
	methods: {
		downloadFile() {
			axios({
				method: "GET",
				url: `http://localhost:3000/documents/_${this.file._id}.${this.file.ext}`,
				responseType: "blob",
			}).then(({ data }) => {
				let a = document.createElement("a");
				a.href = URL.createObjectURL(
					new File([data], this.file.name, { type: types[this.file.ext] })
				);
